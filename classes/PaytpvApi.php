...
class PaytpvApi
{
        ...
        */
        ...
        if (!$b_repeat) {
            ...
            $a_browser_types = [
                ...
                ['msie', true, 'ie', 'bro'],
                ['trident', true, 'ie', 'bro'],
                ['blink', true, 'blink', 'bro'],
                // opera blink: OPR/<number>, no opera in ua
                ['opr/', true, 'blink', 'bro'],
                ['vivaldi', true, 'blink', 'bro'],
                ...
                ['teoma', false, 'ask', 'bot'], // jeeves teoma - leave in this order
            ...
            $a_blink_types = ['opr/', 'vivaldi', 'chromium', 'chrome', 'blink'];
            /**
             * moz types array
            ...
            $a_trident_types = ['ucbrowser', 'ucweb', 'msie'];
         ...
         **/
        if (!$a_engine_data) {
            $a_engine_data = [$layout_engine, $layout_engine_nu_full, $layout_engine_nu];
        }
        if (!$a_blink_data) {
            $a_blink_data = [$blink_type, $blink_type_number, $browser_number];
        }
        if (!$a_khtml_data) {
            $a_khtml_data = [$khtml_type, $khtml_type_number, $browser_number];
        }
        if (!$a_moz_data) {
            $a_moz_data = [$moz_type, $moz_type_number, $moz_rv, $moz_rv_full, $moz_release_date];
        }
        if (!$a_trident_data) {
            $a_trident_data = [$trident_type, $trident_type_number, $layout_engine_nu, $browser_number];
        }
        if (!$a_webkit_data) {
    ...
    public function getHTMLLevel($pv_render_engine, $pv_render_engine_nu)
    {
        ...
        if (
            array_key_exists($pv_render_engine, $a_html5_forms)
            && $a_html5_forms[$pv_render_engine] <= $engine_nu
        ) {
            $html_return = 3;
        } elseif (
            array_key_exists($pv_render_engine, $a_html5_basic)
    ...
    public function validatePaycomet($merchantCode, $idterminal, $password, $terminales)
    {
        ...
        $ret = curl_exec($ch);
        if (!$ret) {
            $ret = json_encode(['DS_RESPONSE' => 2, 'DS_ERROR_ID' => '28']);
        }
        curl_close($ch);
