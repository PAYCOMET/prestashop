		...
		<div class="panel card">
			...
			<div class="card-body">
				<form method="post" class="card-body" action="{$smarty.server.REQUEST_URI|escape:'htmlall':'UTF-8':FALSE}">
					...
					{if $amount>0}
						<p class="center">
							<button type="submit" class="btn btn-default btn-primary" name="submitPayTpvRefund" onclick="this.style.display = 'none'; if (!confirm('{l s='Are you sure?' mod='paytpv'}'))return false;">
								<i class="icon-undo"></i>
						...
						<p class="input-group">
							...
							<span class="input-group-btn">
								<button type="submit" class="btn btn-default btn-primary ml-3" name="submitPayTpvPartialRefund" onclick="this.style.display = 'none'; if (!confirm('{l s='Are you sure?' mod='paytpv'}'))return false;">
									<i class="icon-undo"></i>
...
<fieldset {if isset($ps_version) && ($ps_version < '1.5')}style="width: 400px"{/if}>
	...
	{if $amount>0}
	<p>
		<button type="submit" class="btn btn-default" name="submitPayTpvRefund" onclick="this.style.display = 'none'; if ( !confirm('{l s='Are you sure?' mod='paytpv'}'))return false;">
			<i class="icon-undo"></i>
			{l s='Total Refund of the Payment' mod='paytpv'}  [{$amount|escape:'htmlall':'UTF-8':FALSE}]
		</button>
		{l s='Change order status to Refunded' mod='paytpv'}
	...
	<p>
		...
		<span class="input-group-btn">
			<button type="submit" class="btn btn-default" name="submitPayTpvPartialRefund" onclick="this.style.display = 'none'; if (!confirm('{l s='Are you sure?' mod='paytpv'}'))return false;">
				<i class="icon-undo"></i>
				{l s='Partial Refund of the Payment' mod='paytpv'} [{l s='Máx' mod='paytpv'}.: {$amount|escape:'htmlall':'UTF-8':FALSE}]
			</button>
		</span>
